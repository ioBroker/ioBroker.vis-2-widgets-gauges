...
import ReactBatteryGauge, { type Props as GaugeProps } from 'react-battery-gauge';
...
export default class BatteryGauge extends Generic<BatteryGaugeRxData> {
    ...
    renderWidgetBody(props: RxRenderWidgetProps): React.JSX.Element | React.JSX.Element[] | null {
        ...
        if (
            this.state.rxData.batteryMeterLowBatteryValue !== null &&
            this.state.rxData.batteryMeterLowBatteryValue !== undefined
        ) {
            batteryMeterLowBatteryValue = ((this.state.rxData.batteryMeterLowBatteryValue - min) / (max - min)) * 100;
        }

        const customizationSource: GaugeProps['customization'] = {
            batteryBody: {
            ...
            readingText: {
                lightContrastColor:
                    this.state.rxData.readingTextLightContrastColor || this.props.context.theme.palette.text.primary,
                ...
                fontSize:
                    this.state.rxData.readingTextFontSize === 0
                        ? 0
                        : this.state.rxData.readingTextFontSize || undefined,
        ...
        const customization: GaugeProps['customization'] = {};
        for (const key in customizationSource) {
            for (const subKey in (customizationSource as any)[key]) {
                if ((customizationSource as any)[key][subKey] !== undefined) {
                    (customization as any)[key] ||= {};
                    (customization as any)[key][subKey] = (customizationSource as any)[key][subKey];
                }
            }
        }

        let showValue: null | number;
        let showText = null;
        ...
        const content = (
            ...
            >
                {this.renderCustomText(showText?.toString(), '10%')}
                {size ? (
                    <ReactBatteryGauge
                        value={showValue || 0}
